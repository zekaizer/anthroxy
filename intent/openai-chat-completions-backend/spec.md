...
- ~~RISK1~~ (resolved 2026-09-10: the router strips its own unsigned thinking blocks before an `anthropic` backend, mirroring what Claude Code's `retry:thinking-signature-strip` does after the 400; E16)
...
- E18. Further real runs on 2026-09-10 (Claude Code 2.1.267, LM Studio gemma-4-e2b 32k, Anthropic Haiku/Sonnet): (a) two parallel Read calls in one local turn, then a switch to Haiku that summarised them; (b) a Korean file name and content round-tripped and the local model answered in Korean; (c) MCP tools (`mcp__calc__add` with arguments, `mcp__calc__ping` without) were called and answered on both the local model and Haiku, the argument-less tool reaching the backend with `properties: {}`; (d) a backend outage (config pointed at a closed port, SIGHUP) surfaced as 502 `backend local unreachable after 3 attempt(s)` and the same session resumed after the URL was restored by SIGHUP; (e) a 700-line file read on Haiku then a switch to the 32k local model overflowed its context — LM Studio's 400 was relayed as `invalid_request_error` with its message, Claude Code retried and stopped, and it called `count_tokens` twice in that path (answered 404, tolerated); (f) two concurrent sessions on LM Studio both failed with the same context error while single sessions pass, so LM Studio budgets its loaded context across concurrent requests — a server property, relayed as is. Claude Code assumes a 200k window for an unknown model; `CLAUDE_CODE_MAX_CONTEXT_TOKENS` is its only handle (documented in README), and the gateway model list cannot carry a per-model window. [this session]
- E17. Six-hop switch on 2026-09-10 in one session (local → Haiku → local → Sonnet → Haiku → local, a tool call at most hops, an image read on the Haiku hop): every hop answered on its first request, 10 upstream requests all 200, no 400. Router-made thinking blocks never reached Anthropic; Anthropic's own signed blocks stayed (Haiku kept 1 of its own, Haiku later received 2 of Sonnet's and accepted them); Claude Code itself omitted Haiku's blocks when moving to Sonnet. The image from Haiku's `tool_result` reached the local model as an `image_url` part and it answered about it. [this session]
- E16. Claude Code 2.1.267 stores router-made thinking blocks with `signature: ""` and, on Anthropic's `Invalid signature in thinking block` 400, strips every thinking block and retries (bundle string `[thinking] server rejected a thinking block; stripping all thinking blocks and retrying`, metric `retry:thinking-signature-strip`). With the router stripping unsigned blocks first, the switch back to Haiku answered on the first request. [this session]
