...
- `[stats]` writes `requests-YYYY-MM-DD.jsonl` (UTC dates) owner-only under `stats.dir`: request id, time, requested and routed model and how the name matched (`exact`, `alias` or `default`), backend, upstream model, stream flag, status, attempts, latency, time to first byte, duration, bytes, outcome and token usage (input, output, cache read, cache write). Message content, error bodies and headers are never written. Daily files older than `stats.retention` are deleted with the body log sweep. `count_tokens` and `/v1/models` are not counted.
